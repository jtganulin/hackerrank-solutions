...
numLines = int(input())
# One way to read the block of text from STDIN
lines = "\n".join([input() for i in range(numLines)])

# Get the "valid" emails from the input.
# Take only the unique matches and sort them.
# Finally, separate them with ';' and output.
out = ";".join(sorted(set([i for i in re.findall(r'[\w.]+@[\w.]+\w+', lines)])))
print(out)
